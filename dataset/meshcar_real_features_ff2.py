...
class FaceGraphMeshDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, config, limit_dataset_size=None, single_mode=False, fixed_view_mode=False):
        ...
        self.input_feature_extractor, self.num_feats = {
            ...
            "semantics": (self.input_semantics, 4),
            "normal+semantics": (self.input_normal_semantics, 12)
        }[config.features]
    ...
    def __getitem__(self, idx):
        ...
        real_sample, real_mask, mvp, cam_positions, view_indices, filenames = self.get_image_and_view()
        ...
        normal_maps = pt_arxiv['normal_maps']
        tri_indices_maps = [torch.cat([indices_maps[level][:, [0, 1, 2]], indices_maps[level][:, [0, 2, 3]]], 0) for
                            level in range(len(indices_maps))]
        ranges_maps = [torch.tensor([0, tri_indices_maps[level].shape[0]]).int() for level in
                       range(len(tri_indices_maps))]
        vertex_ctr_maps = [torch.tensor(list(range(vertices_maps[level].shape[0]))).long() for level in
                           range(len(vertices_maps))]
    ...
    def get_image_and_view(self):
        ...
        # available_views = get_car_views() # CompCars
        available_views = get_car_views_nonoise() # ShapeNet
        ...
        image_view_indices = view_indices # aligned views
        # image_view_indices = random.sample(list(range(8)), self.views_per_sample) # unaligned views
        random.seed(self.seed)
        self.seed += 1

        image_selections = [f'{(iidx * 8 + vidx):05d}' for (iidx, vidx) in zip(image_indices, image_view_indices)]

        images, masks, cameras, cam_positions = [], [], [], []
        filenames = []
        for c_i, c_v in zip(image_selections, sampled_view):
            image, filename = self.get_real_image(c_i)
            filenames += [filename]
            images.append(image)
            ...
            # elevation = c_v['elevation'] + 0.0 + (random.random() - 0.5) * self.camera_noise
        ...
        return image, mask, mvp, cam_positions, view_indices, filenames

    def get_real_image(self, name):
        if name not in self.real_images_preloaded.keys():
            filename = self.real_images[name]
            return self.process_real_image(self.real_images[name]), filename
        else:
    ...
    def input_semantics(self, pt_arxiv):
        return torch.nn.functional.one_hot(pt_arxiv['semantics'].long(), num_classes=9).float()
...
def get_car_views():
    ...
    camera_distance = [5.6, 5.6, 2.8, 2.8, 2.2, 2.2, 2.2, 2.2]
    # camera_distance = [3.2, 3.2, 1.7, 1.7, 2.4, 2.4, 2.4, 2.4]
    fov = [10, 10, 40, 40, 40, 40, 40, 40]
    # fov = [40, 40, 40, 40, 40, 40, 40, 40]
    azimuth = [3 * math.pi / 2, math.pi / 2,
               0, math.pi,
               math.pi + math.pi / 3, 0 - math.pi / 3,
               math.pi / 2 + math.pi / 6, math.pi / 2 - math.pi / 6]
    # azimuth_noise = [(random.random() - 0.5) * math.pi / 8, (random.random() - 0.5) * math.pi / 8, # 0
    #                  (random.random() - 0.5) * math.pi / 8, (random.random() - 0.5) * math.pi / 8, # 0
    #                  (random.random() - 0.5) * math.pi / 4, (random.random() - 0.5) * math.pi / 4, # math.pi / 7
    #                  (random.random() - 0.5) * math.pi / 4, (random.random() - 0.5) * math.pi / 4,] # math.pi / 7
    azimuth_noise = [0, 0,
                     0, 0,
    ...
    elevation = [math.pi / 2, math.pi / 2,
                 ...
                 math.pi / 2 - math.pi / 48, math.pi / 2 - math.pi / 48]
    # elevation = [math.pi / 2 + math.pi / 48, math.pi / 2 + math.pi / 48,
    #              math.pi / 2, math.pi / 2,
    #              math.pi / 2 + math.pi / 48, math.pi / 2 + math.pi / 48,
    #              math.pi / 2 + math.pi / 48, math.pi / 2 + math.pi / 48]
    # elevation_noise = [-random.random() * math.pi / 24, -random.random() * math.pi / 24,  # math.pi / 32
    #                    0, 0,
    #                    -random.random() * math.pi / 20, -random.random() * math.pi / 20,  # math.pi / 28
    #                    -random.random() * math.pi / 20, -random.random() * math.pi / 20, ]  # math.pi / 28
...
def get_car_views_nonoise():
    ...
    elevation_noise = [-random.random() * math.pi / 32, -random.random() * math.pi / 32,
                       ...
                       -random.random() * math.pi / 28, -random.random() * math.pi / 28,
                       -random.random() * math.pi / 28, -random.random() * math.pi / 28, ]
    return [{'azimuth': a, 'elevation': e, 'fov': f, 'cam_dist': cd} for a, an, e, en, cd, f in zip(azimuth, azimuth_noise, elevation, elevation_noise, camera_distance, fov)]
