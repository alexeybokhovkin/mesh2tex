...
class ImageNocsScannetDataset(torch.utils.data.Dataset):

    def __init__(self, config, scene_id_range=(0, 0)):
        self.config = config

        self.dataset_directory = Path(config.dataset_path)
        self.dataset_directory = Path('/cluster/valinor/abokhovkin/scannet-texturing/data/ScanNet_chair_masked')
        self.image_size = config.image_size

        target_scene_ids = [x for x in os.listdir(self.dataset_directory) if scene_id_range[0] <= int(x[5:9]) <= scene_id_range[1]]
        self.items = []
        for scene_id in target_scene_ids:
            self.items += list(Path(scene_id, x.stem) for x in Path(self.dataset_directory, scene_id).iterdir() if x.name.endswith('scancrop.jpg'))
    ...
    def __getitem__(self, idx):
        ...
        img_id = str(selected_item).split('_scancrop')[0]
        ...
        noc = np.load(os.path.join(self.dataset_directory, f'{img_id}_noc.npy'))
